...
g <- ggplot(data = CIACountries, aes(y = gdp, x = educ)) + geom_text(aes(label = country, 
color = net_users), size = 3)
...
> ANSWER: reorder(drg, mean_charge) modifies the plot such that the procedures are listed in order of average statewide charge rather than by the numbers that they have been assigned.
...
p <- ggplot(
  data = ChargesNJ, 
  aes(x = drg, y = mean_charge)) + geom_col(fill = "gray") +
...
> ANSWER: The numbers for each DRG are now squished together and labelled horizontally in fairly large font and not properly spaced apart and written vertically as they were in the original graphs.

```{r}
p <- ggplot(
  data = ChargesNJ, 
  aes(x = reorder(drg, mean_charge), y = mean_charge)) + geom_col(fill = "gray") +
  ylab("Statewide Average Charges ($)") + 
  xlab("Medical Procedure (DRG)")
p
```
...
> ANSWER: 
1) How exactly does the "%+%" operator work? The book seems to essentially use it to update the data underlying a plot that we have created without disturbing the aesthetics. Are there other cases where this operator is used?
2) How does one know when to use the "~" operator? It seems to be associated with plotting particular variables against each other, but I'm not sure I am totally clear on its purpose.
