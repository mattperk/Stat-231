...
```{r}
flights_joined2 <- flights_joined %>%
...
```{r}
library(babynames)
babynames2 <- filter(babynames, year>1999)
babynames2
...
> ANSWER: This dataset is called narrow because it summarizes the data using only 3 columns, as compared to the 5 columns associated with the original dataset.
...
> ANSWER: The three names with the largest ratio are Justice, Skyler, and Quinn.
...
> ANSWER: The new BabyNarrow2 has double the number of observations as it includes rows for names which have zero births for a given sex.
