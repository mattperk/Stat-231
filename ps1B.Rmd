...
ggplot(data = MLB_teams) + 
  geom_point(mapping = aes(x = WPct, y = payroll)) +    geom_smooth(aes(x = WPct, y = payroll), method = "lm")
```
...
storms
ggplot(data = storms) + 
  geom_path(aes(x = long, y = lat, color = name)) + scale_color_discrete(guide="none") + facet_wrap(~year)
```
