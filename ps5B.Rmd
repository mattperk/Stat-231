...
> ANSWER: The plot shows that justices have served a wide range of tenures on the Supreme Court, ranging from ~0-40 years. The most common tenure lengths are 0-10 and 10-20 years which have a nearly equal number of justices. 20-30 year tenures are less common, and 30-40 year tenures are fairly rare.
...
justices3 <- justices2 %>%
  select(tenure_length)

ggplot(data = justices3, mapping = aes(x=tenure_length)) +
geom_histogram(binwidth = 10 ,boundary=0, color = "black", fill = "darkblue")
...
```{r, linewidth = 60}
funnyQuotes <- read_csv("quotes.csv")
quotes_random <- funnyQuotes[sample(1:nrow(funnyQuotes), size = 3),]

cat(quotes_random$together)
```
